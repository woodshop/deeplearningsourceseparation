function test_mir1k_general_kl_bss3_td(modelname_in, theta, eI, stage, iter)
...
for ieval = 1:numel(eval_types)
    eval_files = dir([eI.DataPath, filesep, eval_types{ieval}, ...
                      filesep, '*wav']);
    ...
    for ifile = 1:numel(eval_files) % each test songs
        ...
        %%
        % This field is set to 2 in the demo
        if isfield(eI, 'isdiscrim')  && eI.isdiscrim == 2
            [cost, grad, numTotal, pred_cell] = ...
                drdae_discrim_joint_kl_obj(theta, eI, test_data_cell, ...
                                           [],  mixture_spectrum, true, true);
        elseif isfield(eI, 'isdiscrim')  && eI.isdiscrim == 1
            [cost, grad, numTotal, pred_cell] = ...
                drdae_discrim_obj(theta, eI, test_data_cell, [], true, true);
        else
            [cost, grad, numTotal, pred_cell ] = ...
                drdae_obj(theta, eI, test_data_cell, [], true, true);
        end
        ...
        % T x 1
        wavout_noise = source_noise';
        wavout_signal = source_signal';

        Parms = BSS_EVAL(s1, s2, wavout_signal, wavout_noise, mixture);

        if isfield(eI,'bss3') && eI.bss3 == 1
            Parms_bss3 = BSS_3_EVAL(s1, s2, wavout_signal, wavout_noise, ...
                                    mixture);
        else
        ...
        if isfield(eI, 'ioffset')
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                                            strcmp(stage,'iter'))
                    wavwrite(wavout_noise, fs, [eI.saveDir, ...
                                        testname, '_ioff', ...
                                        num2str(eI.ioffset), ...
                                        '_nomask_noise.wav']);
                    wavwrite(wavout_signal, fs, [eI.saveDir, testname, ...
            ...
            fprintf(['%s %s %s iter:%d - no mask - ', ...
                     '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                    modelname, testname, stage, iter, Parms.SDR, ...
                    Parms.SIR, Parms.SAR, Parms.NSDR);
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                % not called by save_callback
                if exist('stage','var') && (strcmp(stage,'done') || ...
                                            strcmp(stage,'iter'))
                    wavwrite(wavout_noise, fs, [eI.saveDir, testname, ...
                                        '_iter', num2str(iter), ...
                                        '_nomask_noise.wav']);
                    wavwrite(wavout_signal, fs, [eI.saveDir, testname, ...
                                        '_iter', num2str(iter), ...
        ...
        if isfield(eI, 'ioffset')
            fprintf(['%s %s %s ioffset:%d iter:%d - binary mask - ', ...
                    ...
                    modelname, testname, stage, eI.ioffset, iter, ...
                    Parms.SDR, Parms.SIR, Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s ioffset:%d iter:%d - binary mask bss3- ', ...
                     '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                     modelname, testname, stage, eI.ioffset, iter, ...
                     Parms_bss3.SDR_bss3, Parms_bss3.SIR_bss3, ...
                     Parms_bss3.SAR_bss3, Parms_bss3.NSDR_bss3);
...
load([savedir, filesep, modelname, filesep, 'model_', num2str(iter),'.mat']);
test_mir1k_general_kl_bss3_td(modelname_in, theta, eI, stage, iter);
