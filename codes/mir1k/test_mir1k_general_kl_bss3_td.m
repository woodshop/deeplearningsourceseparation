function test_mir1k_general_kl_bss3_td(modelname_in, theta, eI, stage,...
    iter)
% Run test and evaluation given a model.

if strcmp(stage,'testall')
    eval_types={'dev','test'};
else
    eval_types={'dev'};
end
...
%         windows=sin(0:pi/winsize:pi-pi/winsize);
        ...
        [test_data_cell, target_ag, mixture_spectrum]= ...
            formulate_data_test_td(mixture, eI, testmode);
        
        %%
        if isfield(eI, 'isdiscrim')  && eI.isdiscrim==2,
            [cost, grad, numTotal, pred_cell] = ...
                drdae_discrim_joint_kl_obj_td(theta, eI, ...
                test_data_cell, [], mixture_spectrum, true, true);
        ...
        %% input
        spectrum.mix = compute_features_td(mixture, eI);
...
%         phase_mix=angle(spectrum.mix);
        ...
        %%
        source_noise = pred_source_noise;
        source_signal = pred_source_signal;
        wavout_noise = compute_inv_features_td(source_noise, eI);
        wavout_signal = compute_inv_features_td(source_signal, eI);
        
        Parms = BSS_EVAL (s1, s2, wavout_signal, wavout_noise, mixture);
        
        if isfield(eI,'bss3') && eI.bss3 == 1
            Parms_bss3 = BSS_3_EVAL(s1, s2, wavout_signal, ...
                wavout_noise, mixture);
        else
            Parms_bss3.SDR_bss3 = 0;
            Parms_bss3.SIR_bss3 = 0;
            Parms_bss3.SAR_bss3 = 0;
            Parms_bss3.NSDR_bss3 = 0;
        end
        
        if isfield(eI,'ioffset'),
            fprintf(['%s %s %s ioffset:%d iter:%d - no mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms.SDR, Parms.SIR, Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s ioffset:%d iter:%d - no mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms_bss3.SDR_bss3, Parms_bss3.SIR_bss3, ...
                Parms_bss3.SAR_bss3, Parms_bss3.NSDR_bss3);
            
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                        strcmp(stage,'iter'))
                    audiowrite(wavout_noise, fs, [eI.saveDir, testname, ...
                        '_ioff', num2str(eI.ioffset), ...
                        '_nomask_noise.wav']);
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname,'_ioff', num2str(eI.ioffset), ...
                        '_nomask_signal.wav']);
                end
            end
        else % finish at once
            fprintf(['%s %s %s iter:%d - no mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter, Parms.SDR, Parms.SIR, ...
                Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s iter:%d - no mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter, Parms_bss3.SDR_bss3, ...
                Parms_bss3.SIR_bss3, Parms_bss3.SAR_bss3, ...
                Parms_bss3.NSDR_bss3);
            
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                        strcmp(stage,'iter')) % not called by save_callback
                    audiowrite(wavout_noise, fs, [eI.saveDir, testname, ...
                        '_iter', num2str(iter), '_nomask_noise.wav']);
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname, '_iter', num2str(iter), ...
                        '_nomask_signal.wav']);
                end
            end
        end
        ...
        gain=1;
        m = double(abs(pred_source_signal) > ...
            (gain*abs(pred_source_noise)));
        ...
        if isfield(eI,'ioffset'),
            fprintf(['%s %s %s ioffset:%d iter:%d - binary mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms.SDR, Parms.SIR, Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s ioffset:%d iter:%d - ', ...
                'binary mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms_bss3.SDR_bss3, Parms_bss3.SIR_bss3, ...
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                        strcmp(stage,'iter'))
                    audiowrite(wavout_noise, fs, [eI.saveDir, testname, ...
                        '_ioff', num2str(eI.ioffset), '_bmask_noise.wav']);
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname, '_ioff', num2str(eI.ioffset), ...
                        '_bmask_signal.wav']);
                end
            ...
            fprintf(['%s %s %s iter:%d - binary mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter, Parms.SDR, Parms.SIR, ...
                Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s iter:%d - binary mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter,  Parms_bss3.SDR_bss3, ...
                Parms_bss3.SIR_bss3, Parms_bss3.SAR_bss3, ...
                Parms_bss3.NSDR_bss3);
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                        strcmp(stage,'iter')) % not called by save_callback
                    audiowrite(wavout_noise, fs, [eI.saveDir, ...
                        testname,'_iter', num2str(iter), ...
                        '_bmask_noise.wav']);
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname, '_iter', num2str(iter), ...
        ...
        %% softmask
        gain=1;
        m = double(abs(pred_source_signal)./(abs(pred_source_signal) + ...
            (gain*abs(pred_source_noise))+eps));
        ...
        if isfield(eI,'ioffset'),
            fprintf(['%s %s %s ioffset:%d iter:%d - soft mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms.SDR, Parms.SIR, Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s ioffset:%d iter:%d - soft mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, eI.ioffset, iter, ...
                Parms_bss3.SDR_bss3, Parms_bss3.SIR_bss3, ...
                Parms_bss3.SAR_bss3, Parms_bss3.NSDR_bss3);
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                    ...
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname, '_ioff', num2str(eI.ioffset), ...
                        '_softmask_signal.wav']);
                end
            end
        else % finish at once
            fprintf(['%s %s %s iter:%d - soft mask - ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter, Parms.SDR, Parms.SIR, ...
                Parms.SAR, Parms.NSDR);
            fprintf(['%s %s %s iter:%d - soft mask bss3- ', ...
                '\tSDR:%.3f\tSIR:%.3f\tSAR:%.3f\tNSDR:%.3f\n'], ...
                modelname, testname, stage, iter, Parms_bss3.SDR_bss3, ...
                Parms_bss3.SIR_bss3, Parms_bss3.SAR_bss3, ...
                Parms_bss3.NSDR_bss3);
            ...
            if isfield(eI,'writewav') && eI.writewav == 1
                if exist('stage','var') && (strcmp(stage,'done') || ...
                        strcmp(stage,'iter')) % not called by save_callback
                    audiowrite(wavout_noise, fs, [eI.saveDir, testname, ...
                        '_iter', num2str(iter), '_softmask_noise.wav']);
                    audiowrite(wavout_signal, fs, [eI.saveDir, ...
                        testname, '_iter', num2str(iter), ...
                        '_softmask_signal.wav']);
                end
    ...
    GNSDR_no_bss3  = sum(GNSDR_bss3.no.*GNSDR_bss3.len) /  ...
        sum(GNSDR_bss3.len);
    GNSDR_binary_bss3 = sum(GNSDR_bss3.binary.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    GNSDR_soft_bss3 = sum(GNSDR_bss3.soft.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    ...
    GSIR_no_bss3  = sum(GSIR_bss3.no.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    GSIR_binary_bss3 = sum(GSIR_bss3.binary.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    GSIR_soft_bss3 = sum(GSIR_bss3.soft.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    ...
    GSAR_no_bss3  = sum(GSAR_bss3.no.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    GSAR_binary_bss3 = sum(GSAR_bss3.binary.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
    GSAR_soft_bss3 = sum(GSAR_bss3.soft.*GNSDR_bss3.len) / ...
        sum(GNSDR_bss3.len);
        
    fprintf('\n');
    if isfield(eI,'ioffset'),
        fprintf(['%s %s ioffset:%d iter:%d - no mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_no, GSIR_no, ...
            GSAR_no);
        fprintf(['%s %s ioffset:%d iter:%d - binary mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_binary, ...
            GSIR_binary, GSAR_binary);
        fprintf(['%s %s ioffset:%d iter:%d - soft mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_soft, GIR_soft, ...
            GSAR_soft);
    else % finish at once
        fprintf(['%s %s iter:%d - no mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, iter, GNSDR_no, GSIR_no, GSAR_no);
        fprintf(['%s %s iter:%d - binary mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'],...
            modelname, stage, iter, GNSDR_binary, GSIR_binary, ...
            GSAR_binary);
        fprintf(['%s %s iter:%d - soft mask - ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, iter, GNSDR_soft, GSIR_soft, GSAR_soft);
    end
    
    if isfield(eI,'ioffset'),
        fprintf(['%s %s ioffset:%d iter:%d - no mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_no_bss3, ...
            GSIR_no_bss3, GSAR_no_bss3);
        fprintf(['%s %s ioffset:%d iter:%d - binary mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_binary_bss3, ...
            GSIR_binary_bss3, GSAR_binary_bss3);
        fprintf(['%s %s ioffset:%d iter:%d - soft mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, eI.ioffset, iter, GNSDR_soft_bss3, ...
            GSIR_soft_bss3, GSAR_soft_bss3);
    else % finish at once
        fprintf(['%s %s iter:%d - no mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, iter, GNSDR_no_bss3, GSIR_no_bss3, ...
            GSAR_no_bss3);
        fprintf(['%s %s iter:%d - binary mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, iter, GNSDR_binary_bss3, ...
            GSIR_binary_bss3, GSAR_binary_bss3);
        fprintf(['%s %s iter:%d - soft mask bss3- ', ...
            '\tGNSDR:%.3f\tGSIR:%.3f\tGSAR:%.3f\n'], ...
            modelname, stage, iter, GNSDR_soft_bss3, GSIR_soft_bss3, ...
            GSAR_soft_bss3);
    end
